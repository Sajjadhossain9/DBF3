...
const SHELL = [BASE, atBase("index.html"), atBase("offline.html"), atBase("manifest.webmanifest")];
...
self.addEventListener("fetch", (e) => {
  ...
  if (req.mode === "navigate") {
    e.respondWith(fetch(req).then((r) => { const copy = r.clone(); caches.open(VERSION).then((c) => c.put(atBase("index.html"), copy)); return r; }).catch(() => caches.match(atBase("index.html")).then((r) => r || caches.match(atBase("offline.html")))));
    return;
  }
